...
namespace Meter.Tests
{
    public abstract class MeterTestBase : AbpIntegratedTestBase<MeterTestModule>
    {
        ...
        protected void UsingDbContext(Action<MeterDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MeterDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MeterDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MeterDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MeterDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MeterDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MeterDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MeterDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MeterDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MeterDbContext, Task<T>> func)
        {
            T result;
